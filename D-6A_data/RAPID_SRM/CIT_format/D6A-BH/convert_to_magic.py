#!/usr/bin/env python
"""Convert the D6A-BH CIT-format demagnetization data to MagIC format.

This script reads the CIT files in this directory (the ``D6A-BH.sam`` site
...
``D-6A_data/RAPID_SRM/MagIC_format/D6A-BH``.

The conversion parameters are read from the ``D6A-BH.inp`` file that sits
...
that produced it. The ``.inp`` file is a three-line, tab-delimited file:
    ...
    ./D6A-BH.sam<tab>SO-V<tab>D6A-BH<tab>5<tab>1<tab>True<tab>None<tab>0.0
...
def main():
    """Run the CIT-to-MagIC conversion for D6A-BH."""
