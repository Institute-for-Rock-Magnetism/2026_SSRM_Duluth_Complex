#!/usr/bin/env python
"""Convert the D6A-AGT_old CIT-format demagnetization data to MagIC format.

This is the same conversion as ``convert_to_magic.py`` but for the alternate
``D6A-AGT_old.sam`` site file (and the per-sample demagnetization files it
references). It writes the five MagIC tables (specimens, samples, sites,
locations, measurements) into
``D-6A_data/RAPID_SRM/MagIC_format/D6A-AGT_old``.

The conversion parameters are read from the ``D6A-AGT_old.inp`` file that sits
alongside the data. The ``.inp`` file is a three-line, tab-delimited file:

    CIT
    sam_path<tab>field_magic_codes<tab>location<tab>naming_convention<tab>...
    ./D6A-AGT_old.sam<tab>SO-V<tab>D6A-AGT<tab>5<tab>1<tab>True<tab>None<tab>0.0

Run it from anywhere with the ``rockmag`` environment active:

    python convert_old_to_magic.py
...
# MagIC_format tree, in a folder named after the .sam file so the D6A-AGT_old
# conversion does not clobber the primary D6A-AGT output.
INPUT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = INPUT_DIR.parents[1] / "MagIC_format" / "D6A-AGT_old"
INP_FILE = INPUT_DIR / "D6A-AGT_old.inp"
...
def main():
    """Run the CIT-to-MagIC conversion for D6A-AGT_old."""
