#!/usr/bin/env python
"""Convert the D6A-felsic CIT-format demagnetization data to MagIC format.

This script reads the CIT files in this directory (the ``D6A-felsic.sam``
...
``D-6A_data/RAPID_SRM/MagIC_format/D6A-felsic``.

The conversion parameters are read from the ``D6A-felsic.inp`` file that
...
file:
    ...
    ./D6A-felsic.sam<tab>SO-V<tab>D6A-felsic<tab>5<tab>1<tab>True<tab>None<tab>0.0
...
INP_FILE = INPUT_DIR / "D6A-felsic.inp"
...
def main():
    """Run the CIT-to-MagIC conversion for D6A-felsic."""
