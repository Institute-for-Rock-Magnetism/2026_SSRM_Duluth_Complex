#!/usr/bin/env python
"""Convert the D6A-ultramafic CIT-format demagnetization data to MagIC format.

This script reads the CIT files in this directory (the ``D6A-ultramafic.sam``
site file and the per-sample demagnetization files it references) and writes
the five MagIC tables (specimens, samples, sites, locations, measurements) into
``D-6A_data/RAPID_SRM/MagIC_format/D6A-ultramafic``.

The conversion parameters are read from the ``D6A-ultramafic.inp`` file that
sits alongside the data, so this script stays in sync with the RAPID SRM
pipeline that produced it. The ``.inp`` file is a three-line, tab-delimited
file:

    CIT
    sam_path<tab>field_magic_codes<tab>location<tab>naming_convention<tab>...
    ./D6A-ultramafic.sam<tab>SO-V<tab>D6A-ultramafic<tab>5<tab>1<tab>True<tab>None<tab>0.0
...
INP_FILE = INPUT_DIR / "D6A-ultramafic.inp"
...
def main():
    """Run the CIT-to-MagIC conversion for D6A-ultramafic."""
